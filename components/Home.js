...
function Home() {
  ...
  useEffect(() => {
    fetch('https://mymovizbackend-xi.vercel.app/movies')
      .then(response => response.json())
      .then(apiData => {
        //setMovies(apiData)
        //console.log(apiData.movies)
        const movieslist = apiData.movies
        const tableautemp = []
        for (let onemovie of movieslist) {
          const img = `https://image.tmdb.org/t/p/w500${onemovie.poster_path}`
          const description = `${onemovie.overview.slice(0, 250)}...`
         
          console.log(onemovie.vote_average)
          const moviesData = ({
            title: onemovie.title,
            poster: img,
            overview: description,
            voteCount: onemovie.vote_count,
            voteAverage: onemovie.vote_average,
          })
          tableautemp.push(moviesData)
          setMovies(tableautemp)
        };
